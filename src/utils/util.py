...
def visualize_with(pattern, map, mc):
    ...
    if pattern == "intensity":
        out = np.zeros((map.shape[0], map.shape[1], map.shape[2], 3))
        
        rgb = np.zeros((map.shape[1], map.shape[2], 3))
        rgb[:,:, 0] = np.reshape(map, (map.shape[1],map.shape[2]))
